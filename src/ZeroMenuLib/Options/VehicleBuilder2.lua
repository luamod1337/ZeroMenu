...
-- Adds a new Vehicle to Attach Vehicle Parent
function updateVehicleList(vehicleData)
  -- Update Attach Vehicle to Vehicle
  print("updateVehicleList(" .. vehicleData['name'] .. " (" .. vehicleData['id'] .. ")")
  for vehicleCount = 1, #spawnedVehicleList do
    local vehicleFeature = spawnedVehicleList[vehicleCount]
    if(vehicleFeature['function'] ~= nil) then
  ...
  -- Update Attach Vehicle to Object  
  
  --objectParentList
  --spawnedObjectList
  for objectCount = 1, #objectParentList.children do
    local objectFeature = objectParentList.children[objectCount]    
    local objectAttachVehicle = objectFeature.children[2]
    --addVehicleFeatureIfNonExisting(vehicleData,objectAttachVehicle,objectParentList[objectCount])
    --menu.add_feature(vehicleData['name'] .. " (" .. vehicleData['hash'] .. ")","action",objectAttachVehicle.id,nil)
    local feature = nil
    feature = menu.add_feature(vehicleData['name'] .. " (" .. vehicleData['id'] .. ")","action_value_str",objectAttachVehicle.id,
      function()
        local offset = v3(0,0,0)
        local rot = v3(0,0,0)
        print("attach " .. vehicleData['id'] .. " to " .. featToEntity[feature]['id'])
        entity.attach_entity_to_entity(vehicleData['id'],featToEntity[feature]['id'],-1,offset,rot,false,true,false,0,true)        
      end)
    --feature.set_str_data(feature,{"center"}) 
    print("id = " .. featToEntity[feature]['id'])
    feature.set_str_data(feature,pb.GetUseAbleBones(featToEntity[feature]['id']))    
  end
end
...
function addObjectFeatureIfNonExisting(vehicle,parent,parentVehicle)
  ...
  if(add) then
    local feature = nil
    feature = menu.add_feature(name,"action_value_str",parent.id,
      function()
        local offset = v3(0,0,0)
        local rot = v3(0,0,0)        
        local bone = entity.get_entity_bone_index_by_name(parentVehicle['id'],vb.GetUseAbleBones(parentVehicle['id'])[feature.value+1])
